  ...
  const handlefetch = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/horseOwner/${id}`,
        {
          headers: {
  ...
  return (
    <div className="container-holder">
      {isPending ? (
        ...
        <>
          <Feed />
          <ProfileHeader image={data && data.avatar} name={name} />
          <Container className="profile-body">
            ...
            <Col className="profile-body-properties">
              ...
              <div className="profile-container">
                ...
                <div className="profile-container-scrollable">
                  <StableCards stable={data && data.horses[0]?.stable} />
                </div>
              </div>
              <ImageGallery />
              <Reviews />
          ...
          <AddHorseModal show={modalShow} onHide={() => setModalShow(false)} />
        </>
      )}
    </div>
  );
